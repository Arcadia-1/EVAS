...
class _ModuleCompiler:
    ...
    def compile(self) -> type:
        ...
        evaluate_ir_laplace_nd_ops = (
            self._collect_evaluate_ir_laplace_nd_ops(mod.analog_block.body)
            if mod.analog_block
            else ()
        )
    ...
    def _collect_evaluate_ir_laplace_nd_ops(
        self,
        stmt,
    ) -> Tuple[Tuple[str, Optional[str], str, Any, Any, Any], ...]:
        """Collect simple first-order continuous laplace_nd voltage contributions."""

        ops: List[Tuple[str, Optional[str], str, Any, Any, Any]] = []
        if not self._collect_evaluate_ir_laplace_nd_ops_from_stmt(stmt, ops):
            return ()
        return tuple(ops)

    def _collect_evaluate_ir_laplace_nd_ops_from_stmt(
        self,
        stmt,
        ops: List[Tuple[str, Optional[str], str, Any, Any, Any]],
    ) -> bool:
        ...
        if isinstance(stmt, Block):
            for child in stmt.statements:
                if not self._collect_evaluate_ir_laplace_nd_ops_from_stmt(child, ops):
                    return False
            return True
        ...
        call = stmt.expr
        if not isinstance(call, FunctionCall) or str(call.name).lower() != "laplace_nd":
            return False
        if len(call.args) < 3:
            return False
